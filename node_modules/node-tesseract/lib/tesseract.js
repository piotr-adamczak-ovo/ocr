...
var Tesseract = {
  ...
  options: {
    'psm': 7,
    'config': __dirname+'/../tessdata/configs/bazaar',
    'binary': 'tesseract'
  },
  ...
  version: function(callback) {
    ...
    var tesseract = spawn(Tesseract.options.binary, ['-v'], {env: productionEnv});

    tesseract.stderr.on('data', function (data) {
      stdout.push(data.toString('utf8'));
    });

    tesseract.on('exit', function (code) {
      if (code == 0 && stdout.length > 0) {

          var verNumber = stdout[0].replace(/\n/g, '');
          verNumber = verNumber.split(" ")[1];
          verNumber = stdout.join(" ");

          callback(null, {"tesseract" : verNumber});
      } else {
          stdout.push("Tesseract error"+code);
   ...
   */
  process: function(image, lcd, options, callback) {
    ...
    // command.push('--user-patterns ' + __dirname + '/eng.user-patterns');
...
module.exports.version = Tesseract.version;